...
class APCA {
    ...
    static getFontSize(fore, back, weight=-1) {//最低限必要なフォントサイズを取得する
        ...
        //const tbl = table.slice(0, table.lastIndexOf(f=>f[0] <= lc)+1).reverse();
        //const tbl = table.slice(0, table.findLastIndex(f=>f[0] <= lc)+1).reverse();
        ...
        /*
        const tbl = table.slice(0, table.findIndex(f=>lc < f[0])+1).reverse();
        const r = tbl[0];
        console.log(table.findLastIndex(f=>f[0] <= lc))
        console.log(table)
        console.log(tbl)
        console.log(r)
        if ([100,200,300,400,500,600,700,800,900].some(v=>v===weight)) {return r[Math.round(weight/100)]}//指定weight該当size返却
        else {return r.slice(1)}// weight 100〜900までの9つのsizeを配列で返す
        */
//        if (weight < 0) {return r.slice(1)} // weight 100〜900までの9つのsizeを配列で返す
//        else {return r[Math.round(weight/100)]} // 指定したweightに該当するsizeだけを返す
    }
}
window.Color = Color;
window.APCA = APCA;
